...
> isDistinct []     = True
> isDistinct [x]    = True
> isDistinct (x:xs) = (not . any (==x) $ xs) && isDistinct xs
...
> genMaybe ga = oneof [Just <$> ga, return Nothing]
...
Too many instances of the empty composite type. We use a frequency distribution to pair
it down.
...
>                         , (n, Branch <$> arbitrary <*> (gen (n `div` 2)) <*> (gen (n `div` 2)))
>                         ]
