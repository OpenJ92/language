...
> foldM f acc (y:ys) = f acc y >>= \acc' -> foldM f acc' ys
...
> sequence [      ] = pure []
> sequence (ma:mas) = ma >>= \a -> sequence mas >>= \as -> pure $ (:) a as
...
> apM mab ma = mab >>= \ab -> ma >>= \a -> pure $ ab a 

> liftM   :: (Monad m) => (a -> b) -> m a -> m b
> liftM f ma = pure f `apM` ma
...
> liftM2 f ma mb = pure f `apM` ma `apM` mb
